...
int CSipServer::Init()
{
    ...
    if (ret != 0) {
        std::cout << "eXosip_init failed, ret=" << ret<< endl;
        return -1;
    }
    std::cout << "eXosip_init successfully!"<< endl;
...
int CSipServer::Start(const std::string &user_agent)
{
    ...
    if (ret != 0) {
        ...
        std::cout << "eXosip_listen_addr failed, ret: " << ret;
        return -1;
    }
    ...
    Infra::CThreadPool::instance()->PushTask(recvTask);
    Infra::CThreadPool::instance()->PushTask(procTask1);

	std::cout << "CSipServer::Start  PushTask recvTask,procTask1" << endl;
...
//    eXosip_unlock(m_excontext);
    ...
    LOG_DEBUG << "Push event: " << sipg_event->name << ", id=" << sipg_event->id << " to queue successfully" << endl;
    return 0;
}
