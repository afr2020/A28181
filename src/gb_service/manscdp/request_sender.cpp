...
int CMsgSender::RegisterOnline(remote_sip_t &remote)
{
    ...
    if (rid < 1) {
        std::cout << "build register failed, " << rid << endl;
        return -1;
    }

	//std::cout << "build register ok, nRet: " <<"g_sipfrom_ipport" << g_sipfrom_ipport <<"g_sipproxy_ipport" << g_sipproxy_ipport<<endl;
	
    eXosip_add_authentication_info(g_excontext, remote.svr_id.c_str(), remote.svr_id.c_str(),
                                   remote.passwd.c_str(), "MD5", nullptr);
	//std::cout<< "remote.svr_id.c_str()" << remote.svr_id.c_str() << "remote.passwd.c_str()" << remote.passwd.c_str() << endl;

    int r = eXosip_register_send_register(g_excontext, rid, reg);
    if (r != 0) {
        std::cout << "send register failed, ret: " << r << endl;
        return -1;
    }

	std::cout << "RegisterOnline ok"<<endl;
    return 0;
}
...
int CMsgSender::send_message(remote_sip_t &remote, const char *body, CBaseRequestSPtr &request)
{
    ...
    if (r != OSIP_SUCCESS) {
        std::cout << "Build message request failed, sipto: " << g_sipproxy_ipport << ", ret: " << r;
        return -1;
    }
    ...
    if (tid < 1) {
        std::cout <<"Send message request failed, sipto: " << g_sipproxy_ipport << ", tid: " << tid;
        return -2;
    }
