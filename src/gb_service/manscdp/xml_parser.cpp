...
int CXmlParser::Load(const char *p_data, int p_size, tinyxml_doc_t &doc)
{
    ...
    if (doc.Error()) {
        std::cout << "Load xml failed, error: "<<doc.ErrorID()<< endl;
        return -1;
    }
    tinyxml_ele_t *ele = doc.RootElement();
    if (nullptr == ele || ele->Value() == nullptr) {
        std::cout << "Load xml failed, error: root element does not exist."<< endl;
        return -2;
    }
    return 0;
}
...
int CXmlParser::ParseXmlHeader(tinyxml_doc_t &doc, manscdp_msgbody_header_t &bodyheader)
{
    ...
    std::cout << "[CXmlParser] ParseXmlHeader, category: " << category_name
              << ", cmdtype: " << cmdtype_name
              << ", devid: "<< bodyheader.devid
              << ", sn: " << bodyheader.sn << endl;
    return 0;
}
...
int CXmlParser::parse_cmdcategory(tinyxml_ele_t *ele, manscdp_cmd_category_e &category, string &name)
{
    ...
    if (nullptr == ele_text) {
        std::cout << "[CXmlMsgParser] parse_cmdcategory failed, error: root element does not have value" << endl;
        return -1;
    }
...
int CXmlParser::ParseEleStr(tinyxml_ele_t *ele, const char *name, string &textstr, bool brequired, bool bto_utf8)
{
    ...
    if (nullptr == child_ele) {
        if (brequired) {
            std::cout << "ParseEleStr: "<< name << " does not exist."<< endl;
        }
    ...
    if (nullptr == ele_text) {
        if (brequired) {
            std::cout << "ParseEleStr: "<<name << "does not have value."<< endl;
        }
        return brequired ? -1 : 0;
    }
    if (bto_utf8) {
...
int CXmlParser::ParseEleInt(tinyxml_ele_t *ele, const char *name, int &value, bool brequired)
{
    ...
    if (nullptr == child_ele) {
        if (brequired) {
            std::cout <<"parse_ele_int: "<<name<< " does not exist."<< endl;
        }
        return brequired ? -1 : 0;
    }
    int ele_value;
    if (child_ele->QueryIntText(&ele_value) != tinyxml2::XML_SUCCESS) {
        if (brequired) {
            std::cout << "parse_ele_int: " <<name<< " has invalid value."<< endl;
        }
        return brequired ? -1 : 0;
    }
    value = ele_value;
